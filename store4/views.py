...
class StripeCompleteView(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        if os.getenv('PAYMENT_DEBUG', False).lower() in ['true', 't', '1'] and data.get('session_id', ''):
    ...
    if request.method == 'POST':
        try:
            ...
            checkout_session = stripe.checkout.Session.create(
                ...
                mode='payment',
                locale='en',
                success_url=f'http://{get_current_site(request=request)}/store' +
                            '/stripe/complete?session_id={CHECKOUT_SESSION_ID}',
                cancel_url=f'http://{get_current_site(request=request)}/store/'  # + '/cancel.html',
...
def StripeCreatePortalSession(request):
    ...
    if not customer:
        return redirect('/store/sgin-in/')

    return_url = f'http://{get_current_site(request=request)}/store/pricing/'
